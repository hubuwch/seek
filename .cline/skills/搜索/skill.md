---
name: 搜索
description: "完整的搜索流程，包含查询压缩、站点绑定和MiniMax web_search搜索。接收自然语言问题，自动压缩查询、绑定权威站点、执行搜索并返回结构化结果。当用户提出需要搜索的问题时，请调用此技能。"
---
...
按照查询压缩 → 站点绑定 → MiniMax搜索步骤，实现从自然语言问题到结构化搜索结果的一站式低成本高精度检索。
...
2. 查询压缩：使用查询压缩工具（scripts/query_compressor.md）提取核心关键词，识别答案类型

3. 站点绑定：使用站点绑定工具（scripts/site_binder.md）根据查询压缩结果和答案类型，为搜索查询添加site约束

4. 搜索：以带site约束的查询参数，调用MiniMax web_search MCP工具，执行搜索，返回结构化JSON结果，结果只包含site约束站点内容
